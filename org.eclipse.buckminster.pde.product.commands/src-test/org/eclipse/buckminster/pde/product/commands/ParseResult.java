...
public class ParseResult {
	...
	static public ParseResult parse(String[] args, List optionDescriptors) throws IllegalArgumentException {
		...
		for (i = 0; i < max; i++) {
			...
			for (int idx = 0; idx < top; ++idx) {
				...
				//
				OptionDescriptor descriptor = (OptionDescriptor) optionDescriptors
						.get(idx);
				...
				//
				if (isLongName
						&& descriptor.isAcceptableName(argName, isLongName,
								false)) {
					if (descriptorToUse != null)
						throw new IllegalArgumentException("Ambiguous option: "
								+ arg);
					descriptorToUse = descriptor;
				}
			}
