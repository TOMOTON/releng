...
public class SdkClasspathContainer implements IClasspathContainer {
  ...
  public static class ContainerEntry {
    ...
    public ContainerEntry(String nameRegExp, boolean copyToWebInfLib, boolean includedInBuildpath) {
   ...
   * type.
   */
  public static IPath computeContainerPath(String containerId, String sdkId, Type containerType) {
    if (containerType == Type.DEFAULT) {
      return computeDefaultContainerPath(containerId);
    } else {
      return computeQualifiedContainerPath(containerId, sdkId);
    }
  }
   ...
   */
  public static IPath computeQualifiedContainerPath(String containerId, String sdkId) {
    assert (containerId != null && containerId.length() > 0);
    assert (sdkId != null);

    return new Path(containerId).append(sdkId);
  }
  ...
  public SdkClasspathContainer(IPath path, String sdkId, IClasspathEntry[] classpathEntries, String description) {
    assert (!path.isEmpty());
    assert (sdkId != null);
